...
FM_overlap_spatial_or_math = np.full((400,), np.nan)

# Mark positions where data_FM is not nan and both data_Spatial and data_Math are nan as 1
FM_overlap_spatial_or_math[np.isfinite(data_FM) & np.isnan(data_Spatial) & np.isnan(data_Math) & np.isnan(data_Asso)] = 1

FM_overlap_spatial_or_math[np.isfinite(data_FM) & (np.isfinite(data_Asso))] = 3

# Mark positions where data_FM is not nan and at least one of data_Spatial or data_Math is not nan as 2
FM_overlap_spatial_or_math[np.isfinite(data_FM) & (np.isfinite(data_Spatial) | np.isfinite(data_Math))] = 2

# %%
fig_FM_overlap_Spatial_or_math = os.path.join(path_output, '%s_Overlap_between_non-semantic_and_feature_matching_difficulty.png'%(fig_num))
plot_Kong_parcellation(FM_overlap_spatial_or_math, fig_FM_overlap_Spatial_or_math, cmap = 'FM', title = 'Overlap between non-semantic and FM difficulty')
