...
scatter_plot_acc = sns.scatterplot(x='11_feature_simi', y='acc', hue='Dataset', data=pd.concat([df_fm_input.assign(Dataset='df_fm_input'), df_non_fm_input.assign(Dataset='df_non_fm_input')]),palette=colors,size=0.6)

sns.regplot(x='11_feature_simi', y='acc', data=df_fm_input, scatter=False, ax=scatter_plot_acc, color=colors['df_fm_input'], label='df_fm_input')
sns.regplot(x='11_feature_simi', y='acc', data=df_non_fm_input, scatter=False, ax=scatter_plot_acc, color=colors['df_non_fm_input'], label='df_non_fm_input')

y_min, y_max=ax.get_ylim()
plt.ylim(y_min, y_max)
plt.xlabel('Feature Similarity', fontproperties=fontprob_label_y)
plt.ylabel('Accuracy',  fontproperties=fontprob_label_y)
...
save_path = os.path.join(current_directory,'../results/Figure_2/Fig_2b_FeatureSimilarity_Accuracy.png')
