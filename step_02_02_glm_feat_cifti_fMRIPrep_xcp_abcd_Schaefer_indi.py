...
from function_save_multiple_images_one_pdf import save_multi_image
from function_plot_parcel_contours import extract_parcel_contours_schaefer
from wbplot import dscalar
from brainspace.utils.parcellation import map_to_labels
from function_Parcellation_to_dense_Glasser import Glasser_parcels_dense
from function_cifti_parcel import save_cifti_ptseries
...
type_interest = types_interest[1]
# check whether first level and second level has run
# if yes, skip first level and second level; if not, run it.
stats_level_1_2 = 12 # 1, has run; others not run
stats_level_3 = 12 # 1 has run
# participants that have some good runs and bad runs
subj_spec = {'sub-18':['1','2','3'],'sub-20':['1','2'],'sub-23':['1','2','3'],'sub-30':['1','2','4'],'sub-33':['1','2','4']}

# participants that have all the bad runs that need to be deleted
subj_delete = ['sub-10','sub-26','sub-02']

# participants ID all
subj_IDs = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-06', 'sub-07', 'sub-08', 'sub-09', 'sub-10', 'sub-12', 'sub-13', 'sub-14', 'sub-15', 'sub-16', 'sub-17', 'sub-18', 'sub-19', 'sub-20R', 'sub-21', 'sub-22', 'sub-23', 'sub-24', 'sub-26', 'sub-28', 'sub-29', 'sub-30','sub-31', 'sub-32', 'sub-33', 'sub-34']
...
trial_types_full = ['correct_no', 'correct_yes', 'wrong_no', 'wrong_yes']

contrasts_types = ['Matching_pm', 'Non-Matching_pm','Matching_and_Non-Matching-pm','Matching_vs_Non-Matching-pm','Non-Matching_vs_Matching-pm',
                   'Matching', 'Non-Matching', 'Matching_and_Non-Matching', 'Matching_vs_Non-Matching','Non-Matching_vs_Matching',
                   'Matching_neg', 'Non-Matching_neg', 'Matching_and_Non-Matching_neg', 'Matching_pm_neg', 'Non-Matching_pm_neg', 'Matching_and_Non-Matching-pm_neg']
            ...
            if not os.path.exists(file_check):
                ...
                for run_ID in run_IDs:
                    ...
                    if os.path.exists(file_fmri):
                        ...
                        # save design matrix
                        path_output_dm = os.path.join(path_output_base, folder_sub_glm_1, subj_ID, ses_ID, parcel_template)
                        ...
                        contrasts = {'Matching': basic_contrasts['correct_yes'],
                                     'Non-Matching': basic_contrasts['correct_no'],
                                     'Matching_and_Non-Matching': basic_contrasts['correct_yes'] + basic_contrasts['correct_no'],
                                     'Matching_vs_Non-Matching': basic_contrasts['correct_yes'] - basic_contrasts['correct_no'],
                                     'Non-Matching_vs_Matching': basic_contrasts['correct_no'] - basic_contrasts['correct_yes'],
                                     'Matching_pm': basic_contrasts['correct_yes_pm'],
                                     'Non-Matching_pm': basic_contrasts['correct_no_pm'],
                                     'Matching_and_Non-Matching-pm': basic_contrasts['correct_yes_pm'] + basic_contrasts['correct_no_pm'],
                                     'Matching_vs_Non-Matching-pm': basic_contrasts['correct_yes_pm'] - basic_contrasts['correct_no_pm'],
                                     'Non-Matching_vs_Matching-pm': basic_contrasts['correct_no_pm'] - basic_contrasts['correct_yes_pm'],
                                     'Matching_neg': - basic_contrasts['correct_yes'],
                                     'Non-Matching_neg': - basic_contrasts['correct_no'],
                                     'Matching_and_Non-Matching_neg': - basic_contrasts['correct_yes'] - basic_contrasts['correct_no'],
                                     'Matching_pm_neg': - basic_contrasts['correct_yes_pm'],
                                     'Non-Matching_pm_neg': - basic_contrasts['correct_no_pm'],
                                     'Matching_and_Non-Matching-pm_neg': - basic_contrasts['correct_yes_pm'] - basic_contrasts['correct_no_pm']}
...
for contrast in contrasts_types:
    ...
    title_base = 'FM-contrast-%s_map-t_FDR-%s' % ( contrast, p_thre_corr)
