...
path_template_dlabel = os.path.join(current_directory, '../Templates/Scene/Scene_Kong_dlabel/Schaefer_417_inflated')
template_dlabel = os.path.join(path_template_dlabel,'Schaefer2018_400Parcels_Kong2022_17Networks_order.dlabel.nii')
...
def write_parcellated_image(
        data, fout, hemisphere=None, cmap_neg='magma', cmap_pos = 'magma', vrange=None):
    """
    ...
    c.set_cmap(data=data, cmap_neg=cmap_neg, cmap_pos = cmap_pos, vrange=vrange)
    c.save(fout)
...
class Cifti(object):
    ...
    def set_cmap(self, data, cmap_neg='magma', cmap_pos = 'magma', vrange=None,  mappable=None):
        ...
        # cmap = check_cmap_plt(cmap)
        # self.vrange = (
        #     np.nanmin(data), np.nanmax(data)) if vrange is None else vrange
        # self.vrange = check_vrange(self.vrange)
        data_max = np.nanmax(data)
        data_min = np.nanmin(data)
        
        if abs(data_min) > abs(data_max):
            data_max = abs(np.nanmin(data))
        elif abs(data_min) < abs(data_max):
            data_min = -np.nanmax(data)
            
        if vrange is None:
            vrange = [np.round(data_min,2), np.round(data_max,2)]
        ...
        if mappable is None:
            ...
            cnorm_neg = clrs.Normalize(vmin=vrange[0], vmax=np.nanmax(data_neg))
            cnorm_pos = clrs.Normalize(vmin=np.nanmin(data_pos), vmax=vrange[1])

            clr_map_neg = cm.ScalarMappable(cmap=cmap_neg, norm=cnorm_neg)
            clr_map_pos = cm.ScalarMappable(cmap=cmap_pos, norm=cnorm_pos)
            
            
            data_cm_neg = np.where(data > 0, np.nan, data)
            data_cm_pos = np.where(data < 0, np.nan, data)
            
            colors_neg = clr_map_neg.to_rgba(data_cm_neg)
            colors_pos = clr_map_pos.to_rgba(data_cm_pos)

            colors = colors_neg+colors_pos
        else:
